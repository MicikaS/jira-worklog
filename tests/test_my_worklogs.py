...
from jira_worklog_pkg.main import my_worklogs
...
class TestMyWorklogsValidation:
    @patch("jira_worklog_pkg.main.JIRA_USERNAME", None)
    def test_no_username_aborts(self):
        with pytest.raises(typer.Abort):
            my_worklogs(month=4, year=2026)

    @patch("jira_worklog_pkg.main.JIRA_USERNAME", "")
...
class TestMyWorklogsApiErrors:
    @patch("jira_worklog_pkg.main.JIRA_USERNAME", "John Doe")
...
class TestMyWorklogsJql:
    @patch("jira_worklog_pkg.main.JIRA_USERNAME", "John Doe")
    @patch("jira_worklog_pkg.main._issue_worklogs", return_value=[])
    @patch("jira_worklog_pkg.main.requests.get")
    ...
    def test_defaults_to_current_month(self, mock_get, mock_worklogs):
        mock_get.return_value = _mock_response(200, {"issues": []})
        with patch("jira_worklog_pkg.main.date") as mock_date:
...
class TestMyWorklogsDisplay:
    @patch("jira_worklog_pkg.main.EMAIL", TEST_EMAIL)
    @patch("jira_worklog_pkg.main.JIRA_USERNAME", "John Doe")
    @patch("jira_worklog_pkg.main._issue_worklogs")
    @patch("jira_worklog_pkg.main.requests.get")
...
class TestMyWorklogsExpectedHours:
    @patch("jira_worklog_pkg.main.WORK_HOURS", {
